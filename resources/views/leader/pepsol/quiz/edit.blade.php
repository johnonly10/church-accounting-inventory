...
@section('content')
    <main class="container-fluid page-shell px-3 px-md-4 py-4">
        <x-page-title title="Edit Quiz" active="Edit Quiz" home="Quizzes" :home-route="route('leader.pepsol-quiz.index')" />
        ...
        <form id="quizForm" action="{{ route('leader.pepsol-quiz.update', $pepsol_quiz) }}" method="POST" novalidate>
            @csrf
            @method('PUT')
                    ...
                    <div class="row">
                            ...
                            <select id="lesson-select" name="pepsol_lesson_id" class="form-select" required>
                                ...
                                @foreach ($lessons as $lesson)
                                    <option value="{{ $lesson->id }}"
                                        {{ old('pepsol_lesson_id', $pepsol_quiz->pepsol_lesson_id) == $lesson->id ? 'selected' : '' }}>
                                        {{ $lesson->title }}
                                        @if ($lesson->pepsol)
                        ...
                        <div class="col-md-6 mb-4">
                            ...
                            <input type="text" id="quiz-title" name="title" class="form-control"
                                placeholder="e.g. Understanding the Holy Spirit Quiz"
                                value="{{ old('title', $pepsol_quiz->title) }}" required>
                        ...
                        <div class="col-12 mb-4">
                            ...
                            <textarea id="quiz-description" name="description" class="form-control" rows="3"
                                placeholder="Brief description of this quiz...">{{ old('description', $pepsol_quiz->description) }}</textarea>
                        </div>
                        ...
                        <div class="col-12 mb-4">
                            ...
                            <textarea id="quiz-instructions" name="instructions" class="form-control" rows="3"
                                placeholder="Instructions for taking this quiz...">{{ old('instructions', $pepsol_quiz->instructions) }}</textarea>
                        </div>
                        ...
                        <div class="col-md-6 mb-6">
                            ...
                            <input type="number" id="passing-score" name="passing_score" class="form-control"
                                min="0" max="100"
                                value="{{ old('passing_score', $pepsol_quiz->passing_score) }}" required>
                        </div>
                        ...
                        <div class="col-md-6 mb-6">
                            ...
                            <input type="number" id="max-attempts" name="max_attempts" class="form-control" min="1"
                                ...
                                value="{{ old('max_attempts', $pepsol_quiz->max_attempts) }}">
                        </div>

                        <div class="col-md-6 mb-4">
                            <div class="form-check">
                                <input type="checkbox" id="allow-retake" name="allow_retake" class="form-check-input"
                                    value="1" {{ old('allow_retake', $pepsol_quiz->allow_retake) ? 'checked' : '' }}>
                                ...
                                <div class="status-pills" id="status-pills">
                                    <label class="status-pill-label" for="status-published" data-status="published">
                                        <input type="radio" id="status-published" name="status" value="published"
                                            {{ old('status', $pepsol_quiz->status) === 'published' ? 'checked' : '' }}>
                                    ...
                                    <label class="status-pill-label" for="status-draft" data-status="draft">
                                        <input type="radio" id="status-draft" name="status" value="draft"
                                            {{ old('status', $pepsol_quiz->status) === 'draft' ? 'checked' : '' }}>
                                        <span class="status-dot" aria-hidden="true"></span>
                                        Draft
                                    </label>
                                </div>
            ...
            <footer class="mt-4">
                <x-buttons.form-action primaryTitle="Update Quiz" primaryId="updateQuizBtn" :cancel-route="route('leader.pepsol-quiz.index')" />
            </footer>
        </form>
    </main>
@endsection
...
@push('scripts')
    ...
    <script>
        // Initialize counters higher than existing IDs to avoid conflicts
        let questionCounter = {{ $pepsol_quiz->questions->count() }};
        let optionCounter = {{ $pepsol_quiz->questions->sum(function ($q) {return $q->options->count();}) }};
        ...
        function addQuestion(questionData = null) {
            questionCounter++;
            const questionNum = questionData ? questionData.num : questionCounter;
            ...
            const reference = questionData ? (questionData.reference || '') : '';
            const points = questionData ? questionData.points : 1;
            const sortOrder = questionData ? questionData.sort_order : (container.children.length);
            const existingId = questionData ? questionData.id : '';

            const questionHtml = `
                <div class="question-card" id="${questionId}" data-question-num="${questionNum}">
                    ${existingId ? `<input type="hidden" name="questions[${questionNum}][id]" value="${existingId}">` : ''}
                    <header class="question-card-header d-flex align-items-center justify-content-between" role="button" tabindex="0" aria-expanded="true">
                        <div class="question-card-title-group">
                            <span class="badge bg-primary">Q${questionNum}</span>
                            <span class="font-weight-bold">Question ${questionNum}</span>
                            <span class="question-card-preview" data-role="preview">${questionText ? `— ${questionText}` : ''}</span>
                        </div>
                        <div class="d-flex align-items-center" style="gap:8px;">
                            <span class="ewm-toggle-icon" style="width:30px;height:30px;" aria-hidden="true">
                                <i class="fas fa-angle-down"></i>
                            </span>
                            <button type="button" class="btn btn-outline-danger btn-sm" data-action="remove-question" aria-label="Remove question ${questionNum}">
                        ...
                        <div class="question-card-body">
                            <div class="row">
                                <div class="col-12 mb-3">
                                    ...
                                    <textarea id="question-text-${questionId}" name="questions[${questionNum}][question_text]" class="form-control" rows="3" placeholder="Enter your question..." required>${questionText}</textarea>
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <textarea id="question-explanation-${questionId}" name="questions[${questionNum}][explanation]" class="form-control" rows="2" placeholder="Explanation shown after answering...">${explanation}</textarea>
                                </div>
                                ...
                                <div class="col-md-4 mb-3">
                                    ...
                                    <input type="text" id="question-reference-${questionId}" name="questions[${questionNum}][reference]" class="form-control" placeholder="e.g. John 3:16" value="${reference}">
                                </div>
                                ...
                                <div class="col-md-2 mb-3">
                                    ...
                                    <input type="number" id="question-points-${questionId}" name="questions[${questionNum}][points]" class="form-control" min="1" value="${points}" required>
                                </div>

                                <div class="col-12">
                                    <input type="hidden" name="questions[${questionNum}][sort_order]" value="${sortOrder}">
            ...
            if (questionData && questionData.options) {
                questionData.options.forEach(option => {
                    addOption(questionId, questionNum, option);
                });
            } else {
                addOption(questionId, questionNum);
                addOption(questionId, questionNum);
            }
        ...
        function addOption(questionId, questionNum, optionData = null) {
            optionCounter++;
            const optionNum = optionData ? optionData.num : optionCounter;
            const optionId = `option_${optionNum}`;
            const container = document.getElementById(`options-${questionId}`);
            const optionCount = container.children.length;

            const optionText = optionData ? optionData.option_text : '';
            const isCorrect = optionData ? optionData.is_correct : false;
            const sortOrder = optionData ? optionData.sort_order : optionCount;
            const existingId = optionData ? optionData.id : '';

            const optionHtml = `
                <div class="option-item ${isCorrect ? 'is-correct' : ''}" id="${optionId}">
                    ${existingId ? `<input type="hidden" name="questions[${questionNum}][options][${optionNum}][id]" value="${existingId}">` : ''}
                    <div class="row align-items-end">
                        <div class="col-md-7 mb-2 mb-md-0">
                            <label for="option-text-${optionId}" class="form-label">Option Text <span class="text-danger">*</span></label>
                            <input type="text" id="option-text-${optionId}" name="questions[${questionNum}][options][${optionNum}][option_text]" class="form-control" placeholder="Enter option text..." value="${optionText}" required>
                            <input type="hidden" name="questions[${questionNum}][options][${optionNum}][sort_order]" value="${sortOrder}">
                        </div>
                        <div class="col-md-3 mb-2 mb-md-0">
                            <div class="form-check">
                                <input type="radio" id="correct-${optionId}" name="questions[${questionNum}][correct_option]" value="${optionNum}" class="form-check-input correct-radio" ${isCorrect ? 'checked' : ''}>
                    ...
                    <input type="hidden" name="questions[${questionNum}][options][${optionNum}][is_correct]" value="${isCorrect ? '1' : '0'}" class="correct-hidden">
                </div>
            `;
        ...
        function populateExistingQuestions() {
            const questions = @json($pepsol_quiz->questions);
            questions.forEach((question, index) => {
                const questionData = {
                    num: index + 1,
                    id: question.id,
        ...
        // Set initial active status pill
        document.addEventListener('DOMContentLoaded', function() {
            const checkedRadio = document.querySelector('#status-pills input[name="status"]:checked');
            if (checkedRadio) {
                const label = checkedRadio.closest('.status-pill-label');
                if (label) label.classList.add('is-active');
            }
        });
        ...
        document.getElementById('quizForm').addEventListener('submit', function(e) {
            ...
            if (submitBtn) {
                ...
                submitBtn.innerHTML = '<i class="fas fa-spinner fa-spin mr-1"></i> Updating Quiz...';
            }
